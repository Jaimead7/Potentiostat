...
from utils import MY_CFG, my_logger
...
class SerialManager:
    ...
    def __post_init__(self) -> None:
        self.cfg: ConfigDict = MY_CFG.serial
    ...
    def setCallbacks(self) -> None:
        self.actionUpdateDevices.triggered.connect(
            lambda: self.actionUpdateDevicesTriggered()
        )
        self.actionConnectDevice.toggled.connect(
            lambda: self.actionConnectDeviceToggled()
        )
        self.serialPort.readyRead.connect(
            lambda: self.serialPortReadyRead()
        )
        self.serialPort.errorOccurred.connect(
            lambda error: self.serialPortErrorOccurred(error)
        )
        self.selector.currentIndexChanged.connect(
            lambda: self.selectorCurrentIndexChanged()
        )
        self.customCmdButton.clicked.connect(
            lambda _: self.sendCustomCmd()
        )
    ...
    def actionUpdateDevicesTriggered(self) -> None:
        ...
        self.parent.debug(self.tr('Serial devices updated'), my_logger.info)

    def actionConnectDeviceToggled(self) -> None:
        if self.actionConnectDevice.isChecked():
            self.parent.debug(
                self.tr(f'Connecting to {self.serialPort.portName()}...')
            )
            self.serialPort.open(QIODevice.OpenModeFlag.ReadWrite)
            self.parent.enableSend(True)
            self.statusBar.setText(self.tr('Connected'))
            self.parent.debug(
                self.tr(f'Configuring {self.serialPort.portName()}...'),
                my_logger.info
            )
            self.parent.sendInitCmds()
            self.parent.debug(
                self.tr(f'{self.serialPort.portName()} initialized'),
                my_logger.info
            )
        else:
            ...
            self.statusBar.setText(self.tr('Disconnected'))
            self.parent.debug(
                self.tr(f'{self.serialPort.portName()} disconnected'),
                my_logger.info
            )
    ...
    def serialPortReadyRead(self) -> None:
        while self.serialPort.canReadLine():
            rcvStr: str = f'{self.serialPort.readLine()}' \
                .replace("b'", "") \
                .replace("'", "") \
                .strip(self.cfg.endCharacter)
            self.parent.debug(
                self.tr(f'Received from {self.serialPort.portName()}: {rcvStr}')
            )
    ...
    def serialPortErrorOccurred(self, error: QSerialPort.SerialPortError) -> None:
        if error != QSerialPort.SerialPortError.NoError:
            self.parent.debug(
                self.tr(
                    f'Error on serial port: {error}->{self.serialPort.errorString()}'
                ),
                my_logger.error
            )
